...
export async function getSpotifyAlbums(ids = []) {
  ...
  const results = await Promise.all(chunks.map(async (chunk) => {
    const data = await fetchSpotifyResource(`/albums?ids=${chunk.join(',')}&market=KR`);
    return data.albums?.filter(Boolean).map(mapAlbum) || [];
  }));
...
export async function getSpotifyAlbumTracks(id) {
  if (!id) return [];
  const data = await fetchSpotifyResource(`/albums/${id}/tracks?market=KR&limit=30`);
  return data.items?.map(mapAlbumTrack) || [];
}
