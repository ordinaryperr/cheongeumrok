import { getCuratedAlbumFallback, getCuratedAlbumId } from '../data/curatedAlbumIds';
import { getSpotifyAlbums, searchSpotify } from './spotify';
...
function mapSpotifyAlbumForCard(album, tags = ['Spotify', '실제 앨범']) {
  return {
    ...
    href: String(album.id).startsWith('search:') ? `/search?q=${encodeURIComponent(album.title)}` : `/albums/spotify/${album.id}`,
    externalUrl: album.externalUrl,
  };
}

function fallbackAlbumFromQuery(query) {
  const curatedFallback = getCuratedAlbumFallback(query);
...
async function searchAlbumFallback(query) {
  try {
    const results = await searchSpotify(query);
    return results.albums?.[0] || fallbackAlbumFromQuery(query);
  } catch {
    return fallbackAlbumFromQuery(query);
  }
}

async function buildAlbumLookup(queries) {
  const curatedEntries = queries
    .map((query) => [query, getCuratedAlbumId(query)])
    .filter(([, id]) => Boolean(id));
  const uniqueIds = Array.from(new Set(curatedEntries.map(([, id]) => id)));
  const lookup = new Map();

  try {
    const albums = await getSpotifyAlbums(uniqueIds);
    const albumsById = new Map(albums.map((album) => [album.id, album]));
    curatedEntries.forEach(([query, id]) => {
      const album = albumsById.get(id) || getCuratedAlbumFallback(query);
      if (album) lookup.set(query, album);
    });
  } catch (error) {
    console.warn('canonical Spotify 앨범 묶음 조회 실패:', error.message);
    curatedEntries.forEach(([query]) => {
      const fallback = getCuratedAlbumFallback(query);
      if (fallback) lookup.set(query, fallback);
    });
  }
...
export async function getSpotifyArchiveCollections() {
  const allQueries = archiveCollections.flatMap((collection) => collection.queries);
  const albumLookup = await buildAlbumLookup(allQueries);

  const collections = await Promise.all(archiveCollections.map(async (collection) => {
    const albums = await Promise.all(collection.queries.map(async (query) => (
      albumLookup.get(query) || await searchAlbumFallback(query)
    )));

    return {
      ...collection,
      albums: albums.map((album) => mapSpotifyAlbumForCard(album, collection.tags.slice(0, 2))),
    };
  }));
...
export async function getSpotifyStarterAlbums() {
  const albumLookup = await buildAlbumLookup(starterAlbumQueries);
  const albums = await Promise.all(starterAlbumQueries.map(async (query) => (
    albumLookup.get(query) || await searchAlbumFallback(query)
  )));
