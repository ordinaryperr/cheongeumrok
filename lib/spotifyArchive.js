import { getCuratedAlbumFallback } from '../data/curatedAlbumIds';
...
export async function getSpotifyArchiveCollections() {
  const collections = archiveCollections.map((collection) => {
    const albums = collection.queries.map((query) => fallbackAlbumFromQuery(query));
    return {
      ...collection,
...
export async function getSpotifyStarterAlbums() {
  return starterAlbumQueries
    .map((query) => fallbackAlbumFromQuery(query))
    .map((album) => mapSpotifyAlbumForCard(album));
}
